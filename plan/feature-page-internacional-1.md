...
- **ASSUMPTION-002**: ~~The three real client countries (Portugal, Inglaterra, EUA) are publishable as stated in CONCEPT §3/§6 and PRODUCT's evidence section. Flagged for her confirmation, not invented here.~~ **Resolved 2026-08-10.** She confirmed the set and extended it: "nos países que atendo adicionar o Canadá e Holanda". Five countries now, carried by the live time-difference list rather than by prose. What is still open is which _city_ stands for Canadá and Holanda on the clock (Toronto and Amsterdã are our default).
- **ASSUMPTION-003**: Brazil observes no daylight saving time (abolished 2019) and Europe and the US still do. The city notes that hedged around this pair are gone (2026-08-10); `domain/clinica/reach` computes the difference at the instant a reader looks, so the assumption is now enforced by `Intl` rather than by careful wording.
