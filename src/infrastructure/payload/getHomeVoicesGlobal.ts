...
export const getHomeVoicesGlobal = cache(
  async function getHomeVoicesGlobal(): Promise<PayloadHomeVoices> {
    const payload = await getPayloadSafe();
    if (!payload) return null;

    const doc = await payload.findGlobal({ slug: "home-voices", depth: 0, overrideAccess: true });
