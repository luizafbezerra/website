 ...
 * a *designed substitute*, never a hidden hole — so this component's contract is
 * that it always renders one of the two.
 *
 ...
 *
 * Before hydration both hooks report their closed value, so the server renders A
 * Lâmina at rest — a complete section of real content. A capable client then
 * upgrades to the Cosmos. The one case that renders nothing is a visitor who has
 ...
 */
export function WowSlot({ content }: { content: Inicio["cosmos"] }) {
  const motionAllowed = useMotionAllowed();
  const wide = useWideViewport();

  if (wide && motionAllowed) return <Cosmos />;

  return <Lamina content={content} />;
}
